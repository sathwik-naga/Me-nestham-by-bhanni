...
import { Save, AlertCircle, CheckCircle2, SlidersHorizontal, Edit3 } from "lucide-react";
...
export default function AdminInventory() {
  ...
  const handleSaveStock = (prod, variantId = null) => {
    const updated = { ...prod };
    if (variantId && updated.variants) {
      updated.variants = updated.variants.map(v => {
        if (v.id === variantId) {
          return { ...v, stock: Number(editVal) };
        }
        return v;
      });
    } else {
      updated.stockCount = Number(editVal);
    }
            ...
            {inventoryRows.map((row, idx) => {
              ...
              return (
                <tr key={idx} className="hover:bg-brand-secondary/35 text-brand-text-muted font-medium">
                  ...
                  <td className="px-6 py-3 capitalize">{row.prod.category.replace("-", " ")}</td>
                  ...
                  <td className="px-6 py-3 text-center">
                    {isEditing ? (
                      <button
                        ...
                        className="p-1.5 bg-brand-primary text-white hover:bg-brand-accent rounded-lg flex items-center justify-center mx-auto"
                      >
                        <Save size={13} />
                      </button>
                    ) : (
                      <button
                        ...
                        className="p-1.5 hover:bg-brand-secondary text-brand-accent rounded-lg border mx-auto flex items-center justify-center"
                      >
                        <Edit3 size={13} />
                      </button>
                    )}
