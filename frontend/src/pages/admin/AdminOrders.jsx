...
import { getOrders, updateOrderStatus } from "../../services/supabase/orders";
import { shippingService } from "../../services/shipping";
import { Save, X, Phone, Eye, Loader2 } from "lucide-react";
...
export default function AdminOrders() {
  ...
  const handleCreateShipment = async () => {
    ...
    try {
      ...
      const res = await shippingService.createShipment(selectedOrder.id);
      alert("Shipment created successfully on Shiprocket.");
      const updatedOrder = { ...selectedOrder, ...res.data.order };
  ...
  const handleGenerateAwb = async () => {
    ...
    try {
      setLogisticsLoading(true);
      const res = await shippingService.generateAwb(selectedOrder.id);
      alert(`AWB Generated: ${res.data.order.awb_code}`);
      const updatedOrder = { ...selectedOrder, ...res.data.order };
      setSelectedOrder(updatedOrder);
      await loadOrdersData();
    } catch (err) {
      alert(err.message || "Failed to generate AWB.");
    } finally {
      setLogisticsLoading(false);
    }
  };
  ...
  const handleSchedulePickup = async () => {
    ...
    try {
      setLogisticsLoading(true);
      const res = await shippingService.schedulePickup(selectedOrder.id);
  ...
  const handleDownloadLabel = async () => {
    ...
    try {
      setLogisticsLoading(true);
      const res = await shippingService.downloadLabel(selectedOrder.id);
      window.open(res.data.order.label_url, "_blank");
      const updatedOrder = { ...selectedOrder, ...res.data.order };
      setSelectedOrder(updatedOrder);
    } catch (err) {
      alert(err.message || "Failed to download label.");
    } finally {
      setLogisticsLoading(false);
    }
  };
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      setLogisticsLoading(true);
      const res = await shippingService.downloadInvoice(selectedOrder.id);
      window.open(res.data.order.invoice_url, "_blank");
      const updatedOrder = { ...selectedOrder, ...res.data.order };
      setSelectedOrder(updatedOrder);
    } catch (err) {
  ...
  const handleDownloadManifest = async () => {
    ...
    try {
      setLogisticsLoading(true);
      const res = await shippingService.downloadManifest(selectedOrder.id);
      window.open(res.data.order.manifest_url, "_blank");
      const updatedOrder = { ...selectedOrder, ...res.data.order };
      setSelectedOrder(updatedOrder);
    } catch (err) {
      alert(err.message || "Failed to download manifest.");
    } finally {
      setLogisticsLoading(false);
    }
  };
  ...
  const handleTrackShipment = async () => {
    ...
    try {
      setLogisticsLoading(true);
      const res = await shippingService.trackShipment(selectedOrder.id);
      alert(`Current Status: ${res.data.tracking_data?.shipment_track?.[0]?.current_status || "Awaiting scan"}`);
      await loadOrdersData();
      const { getOrderDetails } = await import("../../services/supabase/orders");
      const reloadedOrder = await getOrderDetails(selectedOrder.id);
      setSelectedOrder(reloadedOrder);
    } catch (err) {
      alert(err.message || "Failed to track shipment.");
    } finally {
      setLogisticsLoading(false);
    }
  };
  ...
  const handleCancelShipment = async () => {
    if (!selectedOrder) return;
    if (!confirm("Are you sure you want to cancel the Shiprocket shipment?")) return;
    try {
      setLogisticsLoading(true);
      const res = await shippingService.cancelShipment(selectedOrder.id);
      alert("Shipment cancelled successfully on Shiprocket.");
      const updatedOrder = { ...selectedOrder, ...res.data.order };
      setSelectedOrder(updatedOrder);
      await loadOrdersData();
    } catch (err) {
  ...
  const handleCopyTracking = () => {
    navigator.clipboard.writeText(selectedOrder.trackingNumber || selectedOrder.awbCode);
    alert("Tracking number/AWB copied to clipboard.");
  };
  ...
  const handleUpdateStatusSubmit = async (e) => {
    e.preventDefault();
    if (selectedOrder) {
      // Map UI status back to lowercase backend schema ENUM
      let backendStatus = statusVal.toLowerCase();
      ...
      if (backendStatus === "delivered" && selectedOrder.paymentStatus !== "Paid") {
    ...
    return (
      <div className="flex items-center justify-center py-20 font-accent text-brand-text">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs font-semibold text-brand-text-muted">Loading customer orders...</span>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-brand-card border border-brand-border rounded-3xl p-6 md:p-8 shadow-md flex flex-col gap-6">
          <div className="flex items-center justify-between border-b pb-3">
            <h3 className="font-serif font-bold text-lg text-brand-text">
              Manage Order Details: #{selectedOrder.id.substring(0, 8)}...
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {/* Right: Update status controls & Shiprocket Logistics */}
            <div className="flex flex-col gap-6">
              <form onSubmit={handleUpdateStatusSubmit} className="flex flex-col gap-4 bg-brand-secondary/20 p-5 rounded-2xl border">
              ...
              {/* Shiprocket Panel */}
              <div className="flex flex-col gap-4 bg-brand-secondary/20 p-5 rounded-2xl border text-left">
                <span className="font-bold border-b pb-1.5 block text-brand-primary">Shiprocket Logistics Partner</span>
                {logisticsLoading && (
                  <div className="flex items-center gap-1.5 text-[10px] text-brand-primary font-semibold">
                    <Loader2 size={12} className="animate-spin" /> Synchronizing logistics...
                  </div>
                )}
                
                {/* Status info */}
                <div className="grid grid-cols-2 gap-3 text-[10px] text-brand-text-muted leading-relaxed">
                  <div>
                    <span className="font-bold text-brand-text block">Shipping Status</span>
                    <span className="capitalize">{selectedOrder.shippingStatus || "Not Created"}</span>
                  </div>
                  <div>
                    <span className="font-bold text-brand-text block">Pickup Status</span>
                    <span className="capitalize">{selectedOrder.pickupStatus || "Pending"}</span>
                  </div>
                  <div>
                    <span className="font-bold text-brand-text block">Courier</span>
                    <span>{selectedOrder.courierName || "None Assigned"}</span>
                  </div>
                  <div>
                    <span className="font-bold text-brand-text block">AWB Code</span>
                    <span className="font-mono">{selectedOrder.awbCode || "Awaiting Assignment"}</span>
                  </div>
                  {selectedOrder.estimatedDelivery && (
                    <div className="col-span-2">
                      <span className="font-bold text-brand-text block">Estimated Delivery</span>
                ...
                <div className="flex flex-wrap gap-2.5 pt-3 border-t mt-1">
                  {!selectedOrder.shipmentId ? (
                    <button
                      ...
                      className="bg-brand-primary hover:bg-brand-accent text-white px-4 py-2 rounded-xl shadow-sm font-semibold text-[10px] cursor-pointer disabled:opacity-50"
                    >
                      Create Shipment
                    </button>
                  ) : (
                    <>
                      ...
                      {selectedOrder.awbCode && selectedOrder.pickupStatus !== 'scheduled' && (
                        <button
                          type="button"
                      ...
                      {selectedOrder.awbCode && (
                        <button
                          ...
                          disabled={logisticsLoading}
                          className="bg-brand-accent text-white px-4 py-2 rounded-xl shadow-sm font-semibold text-[10px] cursor-pointer disabled:opacity-50"
                        >
                          Track Shipment
                        </button>
                      )}
                      ...
                      {selectedOrder.shippingStatus !== 'cancelled' && (
                        <button
                          ...
                          disabled={logisticsLoading}
                          className="bg-brand-error/15 hover:bg-brand-error hover:text-white text-brand-error border border-brand-error/30 px-4 py-2 rounded-xl text-[10px] font-semibold cursor-pointer disabled:opacity-50"
                        >
                          Cancel Shipment
                        </button>
                      )}
                ...
                {/* Print and Documents Links */}
                {selectedOrder.shipmentId && (
                  <div className="flex flex-wrap gap-2.5 border-t pt-3 text-[10px] font-semibold text-brand-accent">
                    <button type="button" onClick={handleDownloadLabel} className="hover:underline">
                      {selectedOrder.labelUrl ? "Re-download Label" : "Download Label"}
                    </button>
                    <button type="button" onClick={handleDownloadInvoice} className="hover:underline">
                      {selectedOrder.invoiceUrl ? "Re-download Invoice" : "Download Invoice"}
                    </button>
                    <button type="button" onClick={handleDownloadManifest} className="hover:underline">
                      Download Manifest
                    </button>
                  </div>
                )}

                {/* Quick actions for AWB */}
                {selectedOrder.awbCode && (
                  <div className="flex flex-wrap gap-2.5 border-t pt-3 text-[10px] font-semibold text-brand-primary">
                    <button type="button" onClick={handleCopyTracking} className="hover:underline">
                      Copy Tracking Number
                    </button>
                    {selectedOrder.trackingUrl && (
                      <a href={selectedOrder.trackingUrl} target="_blank" rel="noopener noreferrer" className="hover:underline">
                        Open Tracking URL
                      </a>
                    )}
                  </div>
              ...
              <tbody className="divide-y divide-brand-border/60">
                {orders.map((ord) => (
                  <tr key={ord.id} className="hover:bg-brand-secondary/35 text-brand-text-muted font-medium">
                    <td className="px-6 py-3 font-mono font-bold text-brand-primary">{ord.id.substring(0, 8)}...</td>
                    <td className="px-6 py-3 font-serif font-bold text-brand-text">{ord.shippingAddress?.fullName || "Guest Customer"}</td>
                    <td className="px-6 py-3">{ord.date ? new Date(ord.date).toLocaleDateString() : ""}</td>
