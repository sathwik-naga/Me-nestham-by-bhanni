...
import { User, ShieldAlert, CheckCircle2, Eye, Phone, Mail } from "lucide-react";
...
export default function AdminCustomers() {
  ...
  const handleToggleStatus = (cust) => {
    ...
    if (window.confirm(`Are you sure you want to change the status of ${cust.name} to ${nextStatus}?`)) {
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl font-semibold shadow-sm transition-all border ${
                    selectedCust.status === "Active" 
                      ? "bg-brand-error/15 border-brand-error/30 text-brand-error hover:bg-brand-error hover:text-white" 
                      : "bg-brand-success/15 border-brand-success/30 text-brand-success hover:bg-brand-success hover:text-white"
                  }`}
                >
                  {selectedCust.status === "Active" ? "Suspend Account" : "Activate Account"}
                </button>
              </div>
            </div>
          </div>
